...
{
    public delegate void Middleware(HttpListenerContext context, Action next);
    ...
    {
        int index = 0;
        Action next = null; // Suppress compiler warning (if needed)
        ...
        next(); // Safely initialized before use
    }
}
"@

# Compile the C# code in PowerShell
Add-Type -TypeDefinition $csCode -Language CSharp

# Create an instance of the MiddlewareHandler class
$middlewareHandler = New-Object MiddlewareHandler

# Define Middleware Functions in PowerShell
$loggingMiddleware = {
    param($context,$next)
    Write-Host "Middleware Processing request: $($context.Request.RawUrl)"
    $next.invoke()
}

$authMiddleware = {
    param($context,$next)
    if (-not $context.Request.Headers["Authorization"]) {
...
$middlewareHandler.Use($loggingMiddleware)
...
while($Httplistener.IsListening){
    foreach($Event in @(Get-Event HTTP*)){
        ...
        $middlewareHandler.Process($context)
